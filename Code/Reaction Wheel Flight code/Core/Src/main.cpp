...
int main(void)
{
  ...
  /* Configure the system clock */
    ...
    double max_dc = 0.08;
    double min_dc = 0.06;
    double deadzone_start = 0.067;
    double deadzone_end = 0.0724;
    double neutral_point = 0.5 * (deadzone_start + deadzone_end);
    double dc = 0.0;
    */

    Reversable_PWM_Motor* motor = new Reversable_PWM_Motor(
  	&htim2,        // Timer
  	TIM2,          // Timer address
    ...
    motor->arm();
    //HAL_StatusTypeDef read_res = HAL_I2C_Mem_Read(&hi2c1, readAddress, offset + i, 2, &read_byte, 1, 1000);
    //uint8_t CARGO_NO = 1;
